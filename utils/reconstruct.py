...
def make_obmol(xyz, atomic_numbers, atom_affinity):
    mol = ob.OBMol()
    mol.BeginModify()
    atoms = []
    for xyz, t, affinity in zip(xyz, atomic_numbers, atom_affinity):
        x, y, z = xyz
        ...
        # atom.SetData("affinity_weight", affinity)
        pd = ob.OBPairData()
        pd.SetAttribute('_affinity_weight')
        pd.SetValue(str(affinity))
        atom.CloneData(pd)
...
def convert_ob_mol_to_rd_mol(ob_mol, struct=None):
    ...
    for ob_atom in ob.OBMolAtomIter(ob_mol):
        ...
        # set affinity contribution
        rd_atom.SetProp('_affinity_weight', ob_atom.GetData('_affinity_weight').GetValue())
...
def reconstruct_from_generated(xyz, atomic_nums, aromatic=None, atom_affinity=[], basic_mode=True):
    """
