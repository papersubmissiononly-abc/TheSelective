...
class NormalizeVina(object):
    ...
    def __init__(self, mode='pl'):
        ...
        if mode == 'pl':
            ...
            self.max_v = 0
            self.min_v = -16
        elif mode == 'crossdock2020':
            # CrossDock2020 uses same normalization as pl
            self.max_v = 0
            self.min_v = -16
        else:
            raise ValueError
    ...
    def _trans(self, vina_score):
        if self.mode == 'pl' or self.mode == 'crossdock2020':
...
class NormalizeVinaAndPair(object):

    def __init__(self, mode='multipro'):
        super().__init__()
        self.mode = mode
        if mode == 'multipro':
            self.max_v = 0
    ...
    def __call__(self,  data: ProteinLigandData):
        data.affinity = self._trans(data.affinity)
        data.on_target_affinity = self._trans(data.on_target_affinity)
        data.off_target_affinities = self._trans(data.off_target_affinities)
        return data
