...
class PDBProtein(object):
    ...
    def _enum_formatted_atom_lines(self):
        for line in self.block.splitlines():
            if line[0:6].strip() == 'ATOM':
                ...
                # For PDBQT files, take the first character of the atom name as the element
                if len(element_symb) > 1 and element_symb[1].isdigit():
                    element_symb = element_symb[0]
                yield {
                    'line': line,
                    'type': 'ATOM',
    ...
    def _parse(self):
        ...
        for atom in self._enum_formatted_atom_lines():
            ...
            element_symb = atom['element_symb']
            if element_symb == 'A':
                element_symb = 'C'
            elif len(element_symb) > 1:
                element_symb = element_symb[0]
    ...
    def _recalculate_residue_properties(self):
        # This method assumes self.pos has been updated.
        self.center_of_mass = []
        self.pos_CA = []
        self.pos_C = []
        self.pos_N = []
    ...
    def translate(self, vec):
        ...
        self._recalculate_residue_properties()
