...
class VinaDockingTask(BaseDockingTask):
    ...
    def __init__(self, protein_path, ligand_rdmol, tmp_dir='./tmp', center=None,
        ...
        # Validate ligand conformer and positions
        ...
        # Check if positions array is empty or malformed
        if pos.size == 0:
            raise ValueError("Ligand conformer has no atom positions")
        if len(pos.shape) != 2 or pos.shape[1] != 3:
            raise ValueError(f"Invalid position array shape: {pos.shape}, expected (n_atoms, 3)")
    ...
    def run(self, mode='dock', exhaustiveness=8, save_path=None, **kwargs):
...
#
#     dock = VinaDock(ligand_pdbqt, prot_pdbqt)
#     dock.get_box()
#     dock.dock()