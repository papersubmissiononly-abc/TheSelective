#!/usr/bin/env python3
"""
Evaluate generated molecules from TheSelective diffusion model.
Computes validity, stability, QED, SA, Vina docking scores, and bond/atom distributions.

Usage:
    python scripts/evaluate_diffusion.py --sample_path ./results/theselective_id0_50
    python scripts/evaluate_diffusion.py --sample_path ./results/theselective_id0_50 --docking_mode vina_dock
"""
...
def main():
    ...
    parser.add_argument('--save_complex', action='store_true', default=False,
                        help='Save protein-ligand complex')
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        ...
        for sample_idx, (pred_pos, pred_v, pred_exp_score, pred_exp_atom_weight) in enumerate(
                zip(all_pred_ligand_pos, all_pred_ligand_v, all_pred_exp_score, all_pred_exp_atom_traj)):
            pred_pos, pred_v = pred_pos[args.eval_step], pred_v[args.eval_step]
            pred_exp = pred_exp_score
            pred_exp_atom_weight = pred_exp_atom_weight[args.eval_step]
            ...
            try:
                ...
                mol = reconstruct.reconstruct_from_generated(
                    pred_pos, pred_atom_type, pred_aromatic, pred_exp_atom_weight)
            ...
            # Chemical properties and docking
            ...
            if args.docking_mode == 'qvina':
                ...
                if args.docking_mode == 'vina_dock':
                    ...
                    if args.save_complex:
                        ...
                        docking_save_path = os.path.join(
                            complex_path, f'{example_idx}_{sample_idx}_dock.pdb')
                    docking_results = vina_task.run(mode='dock',
                                                    exhaustiveness=args.exhaustiveness,
                                                    save_path=docking_save_path)
            ...
            results.append({
                ...
                'pred_exp': pred_exp,
                'atom_exp': {
                    atom.GetIdx(): parse_affinity_weight(atom.GetProp('_affinity_weight'))
